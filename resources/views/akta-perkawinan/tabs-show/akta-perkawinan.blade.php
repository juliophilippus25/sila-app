<div class="space-y-2">
    ...
    <div class="flex">
        ...
        <span class="text-gray-900">:
            {{ $aktaPerkawinan->tanggal_cetak ? tglIndo($aktaPerkawinan->tanggal_cetak) : '-' }}
        </span>
    </div>
    <div class="flex">
    ...
    @if($aktaPerkawinan->status == 'rejected')
        <div class="flex">
            <span class="w-64 font-semibold text-gray-700">Alasan Penolakan</span>
            <span class="text-gray-900 capitalize">: {{ $aktaPerkawinan->alasan ?? '-' }}</span>
        </div>
    @endif
</div>
